...
1. Tải file `.zip` mới nhất từ trang [Releases](../../releases).
2. Giải nén vào bất kỳ đâu (ví dụ: `C:\Tools\ABA-Anti-lag\`).
3. Chạy `RegionBlocker.exe` — Windows sẽ hỏi quyền Administrator. Nhấn **Yes**.

> **Lưu ý:** Phải cài `.NET 8 Desktop Runtime`. Nếu khởi động báo lỗi "you must install .NET", tải runtime từ link trên rồi chạy lại.

---

## Thiết lập lần đầu

![Giao diện chính của Region Blocker](screenshots/main_ui.png)
...
![Trạng thái khi trigger đã kích hoạt](screenshots/triggered_ui.png)
...
![Preview window với detect points](screenshots/preview_window.png)
